...
//       {Array.isArray(complaient) && complaient.length > 0 ? (
//         <div className='mb-96'>
//           <table className="table-auto  text-white mt-4 mr-2">
//             <thead>
  ...
  useEffect(() => {
    const fetchTharavData = async () => {
      try {
        ...
        // Filter the data where grampanchyatName is 'फलटण'
        const filteredData = data.filter(record => {
          const grampanchyatName = record.grampanchyatName?.trim();
          console.log('Trimmed grampanchyatName:', grampanchyatName); // Log trimmed grampanchyatName value
          return grampanchyatName && grampanchyatName.normalize('NFKC') === 'फलटण'.normalize('NFKC');
        });

        console.log('Filtered Data:', filteredData); // Log filtered data

        setComplaient(filteredData); // Set the filtered data
      } catch (err) {
        setError(err.message);
      } finally {
        setLoading(false);
