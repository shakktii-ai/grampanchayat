...
//  
  ...
  const filteredCertificates = (() => {
    ...
    return certificatesToFilter.filter(cert => cert.status === 'प्रक्रिया पूर्ण झाली');
  })();
  ...
  {selectedCertificate?.grampanchayantUploadDocument && (
                      <img
                        src={selectedCertificate?.grampanchayantUploadDocument}
                        alt="grampanchayantUploadDocument"
                        className="h-32 w-auto object-contain mb-2"
                        onClick={() => openImageModal(selectedCertificate?.grampanchayantUploadDocument)}
                      />
                    )}</div>
        <button
          type="button"
