...
import Developmentwork from '../../models/Developmentwork';
...
export default async function handler(req, res) {
  if (req.method === 'POST') {
    try {
      ...
      const developmentwork = new Developmentwork(req.body);

      // Save the record to the database
      await developmentwork.save();

      // Send a response back
      res.status(201).json({ message: 'Form submitted successfully', data: developmentwork });
    } catch (error) {
      // console.error(error);
    ...
    try {
      ...
      const developmentwork = await Developmentwork.find();

      // Send the data in the response
      res.status(200).json({ message: 'marriage certificates fetched successfully', data: developmentwork });
    } catch (error) {
      console.error(error);
      res.status(500).json({ message: 'Internal server error' });
    }
    ...
    try {
      ...
      const updatedCertificate = await Developmentwork.findByIdAndUpdate(
        id,
        { status },
