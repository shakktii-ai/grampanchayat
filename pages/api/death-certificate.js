...
export default async function handler(req, res) {
  if (req.method === 'POST') {
    try {
      ...
      const deathCertificate = new DeathCertificate(req.body);

      // Save the record to the database
      await deathCertificate.save();

      // Send a response back
      res.status(201).json({ message: 'Form submitted successfully', data: deathCertificate });
    } catch (error) {
      console.error(error);
    ...
    try {
      ...
      const deathCertificate = await DeathCertificate.find();

      // Send the data in the response
      res.status(200).json({ message: 'marriage certificates fetched successfully', data: deathCertificate });
    } catch (error) {
      console.error(error);
      res.status(500).json({ message: 'Internal server error' });
    }
