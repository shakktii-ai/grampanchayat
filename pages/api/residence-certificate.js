...
export default async function handler(req, res) {
  if (req.method === 'POST') {
    try {
      ...
      const residenceCertificate = new ResidenceCertificate(req.body);

      // Save the record to the database
      await residenceCertificate.save();

      // Send a response back
      res.status(201).json({ message: 'Form submitted successfully', data: residenceCertificate });
    } catch (error) {
      console.error(error);
    ...
    try {
      ...
      const residenceCertificate = await ResidenceCertificate.find();

      // Send the data in the response
      res.status(200).json({ message: 'marriage certificates fetched successfully', data: residenceCertificate });
    } catch (error) {
      console.error(error);
      res.status(500).json({ message: 'Internal server error' });
    }
    ...
    try {
      ...
      const updatedCertificate = await ResidenceCertificate.findByIdAndUpdate(
        id,
        { status },
